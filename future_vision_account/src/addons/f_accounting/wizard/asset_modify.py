...
class AssetModify(models.TransientModel):
    ...
    asset_id = fields.Many2one(string="Asset", comodel_name='asset', required=True, help="The asset to be modified by this wizard", ondelete="cascade")
    ...
    def _compute_accounts(self):
        for record in self:
            record.gain_account_id = record.company_id.account_journal_early_pay_discount_gain_account_id 
            record.loss_account_id = record.company_id.account_journal_early_pay_discount_loss_account_id 

    def _inverse_gain_account(self):
        for record in self:
            record.company_id.sudo(). account_journal_early_pay_discount_gain_account_id  = record.gain_account_id

    def _inverse_loss_account(self):
        for record in self:
            record.company_id.sudo(). account_journal_early_pay_discount_loss_account_id  = record.loss_account_id
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'asset_id' in vals:
                asset = self.env['asset'].browse(vals['asset_id'])
    ...
    def modify(self):
        ...
        if self.currency_id.round(residual_increase + salvage_increase) > 0:
            ...
            move._post()
            asset_increase = self.env['asset'].create({
        ...
        tracked_fields = self.env['asset'].fields_get(old_values.keys())

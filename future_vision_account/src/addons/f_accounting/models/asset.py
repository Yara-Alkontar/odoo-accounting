...
class Asset(models.Model):
    ...
    salvage_value = fields.Monetary(string = 'Not Depreciable Value', readonly = True, states = {'draft': [('readonly', False)]},  help="It is the amount you plan to have that you cannot depreciate.")                               
    ...
    model_id = fields.Many2one('asset', string = 'Model', change_default = True, readonly = True, states = {'draft': [('readonly', False)]}, domain="[('company_id', '=', company_id)]")
    ...
    # Capital gain
    parent_id = fields.Many2one('asset', help = "An asset has a parent when it is the result of gaining value")
    children_ids = fields.One2many('asset', 'parent_id', help = "The children are the gains in value of this asset")
    ...
    def create(self, vals_list):
        for vals in vals_list:
            ...
            if self._context.get('default_state') != 'model' and vals.get('state') != 'model':
                vals['state'] = 'draft'
        new_recs = super(Asset, self.with_context(mail_create_nolog=True)).create(vals_list)
        ...
        if self.env.context.get('original_asset'):
            ...
            original_asset = self.env['asset'].browse(self.env.context.get('original_asset'))
    ...
    def action_save_model(self):
        form_ref = {
            'purchase': 'f_accounting.view_f_accounting_asset_form',
            'sale': 'f_accounting.view_f_accounting_asset_revenue_form',
            'expense': 'f_accounting.view_f_accounting_asset_expense_form',
        }.get(self.asset_type)
    ...
    def open_increase(self):
        return {
            ...
            'views': self.env['asset']._get_views(self.asset_type),
        }
    ...
    def validate(self):
        ...
        ref_tracked_fields = self.env['asset'].fields_get(fields)
    ...
    def _get_views(self, asset_type):
        form_view = self.env.ref('f_accounting_asset.view_f_accounting_asset_form')
        tree_view = self.env.ref('f_accounting_asset.view_f_accounting_asset_purchase_tree')
        if asset_type == 'sale':
            form_view = self.env.ref('f_accounting_asset.view_f_accounting_asset_revenue_form')
            tree_view = self.env.ref('f_accounting_asset.view_f_accounting_asset_sale_tree')
        elif asset_type == 'expense':
            form_view = self.env.ref('f_accounting_asset.view_f_accounting_asset_expense_form')
            tree_view = self.env.ref('f_accounting_asset.view_f_accounting_asset_expense_tree')
    ...
    def _get_disposal_moves(self, invoice_lines_list, disposal_date):
        ...
        for asset, invoice_line_ids in zip(self, invoice_lines_list):
            ...
            difference_account = asset.company_id.account_journal_early_pay_discount_gain_account_id  if difference > 0 else asset.company_id.account_journal_early_pay_discount_loss_account_id

...
class AccountMove(models.Model):
    ...
    asset_id = fields.Many2one('asset', string='Asset', index=True, ondelete='cascade', copy=False, domain="[('company_id', '=', company_id)]")
    ...
    asset_ids = fields.One2many('asset', string = 'Assets', compute = "_compute_asset_ids")
        ...
        for move in self:
            ...
            if asset:
                if asset.currency_id == asset.company_id.currency_id:
                    move._compute_depreciation_by_curr('debit', 'credit', 'balance')
                else:
                    move._compute_depreciation_by_curr('debit', 'credit', 'balance')
                    move.depreciation_value = move.company_id.currency_id._convert(move.depreciation_value,asset.currency_id, move.company_id, move.date or fields.Date().today())                                                                          
            else:
                asset_depreciation = 0
                move.depreciation_value = asset_depreciation
    
    def _compute_depreciation_by_curr(self, debit_field, credit_field, balance_field):
        account = asset.Asset.account_depreciation_expense_id if asset.Asset.asset_type != 'sale' else asset.account_depreciation_id
        asset_depreciation = sum(
         self.asset_ids.original_move_line_ids.filtered(lambda l: l.account_id == account).mapped(balance_field)
        )
        
        # Special case of closing entry - only disposed assets of type 'purchase' should match this condition
        if any(
            line.account_id == asset.Asset.account_asset_id
            and float_compare(-line.balance, asset.original_value, precision_rounding=asset.currency_id.rounding) == 0
            for line in self.asset_ids.original_move_line_ids
            
        ):
            account = asset.Asset.account_depreciation_id
            asset_depreciation = (
                asset.original_value
                - asset.salvage_value
                - sum(
                  self.asset_ids.original_move_line_ids.filtered(lambda l: l.account_id == account).mapped(
                        debit_field if asset.original_value > 0 else credit_field
                    )
    ...
    def button_cancel(self):
        ...
        self.env['asset'].sudo().search([('original_move_line_ids.move_id', 'in', self.ids)]).write({'active': False})
        return res
    ...
    def _auto_create_asset(self):
        ...
        assets = self.env['asset'].create(create_list)
...
class AccountMoveLine(models.Model):
    ...
    asset_ids = fields.Many2many('asset', 'asset_move_line_rel', 'line_id', 'asset_id', string='Related Assets', copy=False)
    ...
    def turn_as_asset(self):
        return self._turn_as_asset('purchase', _("Turn as an asset"), self.env.ref("f_accounting_asset.view_f_accounting_asset_form"))

    def turn_as_deferred(self):
        balance = sum(aml.debit - aml.credit for aml in self)
        if balance > 0:
            return self._turn_as_asset('expense', _("Turn as a deferred expense"), self.env.ref('f_accounting_asset.view_f_accounting_asset_expense_form'))
        else:
            return self._turn_as_asset('sale', _("Turn as a deferred revenue"), self.env.ref('f_accounting_asset.view_f_accounting_asset_revenue_form'))
